<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:1a1023,50:3d1a2e,100:1a1023&height=220&section=header&text=Ela%20Toumi&fontSize=60&fontColor=ff8fc4&animation=fadeIn&fontAlignY=38&desc=AI%20Engineering%20Student%20·%20LLMs%20%26%20Agentic%20Systems&descAlignY=58&descSize=18&descColor=f4c6dc" width="100%"/>

<br/>

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=20&duration=3000&pause=800&color=FF7EB6&center=true&vCenter=true&width=600&lines=%5BSYSTEM%5D+AI+Engineering+Student+%7C+Tunis%2C+Tunisia+%F0%9F%87%B9%F0%9F%87%B3;Building+AI+platforms+%40+ESPRIT+%C3%97+IRA+M%C3%A9denine;LLMs+%2B+Multi-Agent+Systems+%2B+XAI" alt="Typing SVG" />

<br/><br/>

<a href="https://www.linkedin.com/in/YOUR-LINKEDIN-HANDLE"><img src="https://img.shields.io/badge/LinkedIn-ff8fc4?style=for-the-badge&logo=linkedin&logoColor=1a1023" /></a>
<a href="mailto:YOUR_EMAIL@gmail.com"><img src="https://img.shields.io/badge/Gmail-f472b6?style=for-the-badge&logo=gmail&logoColor=1a1023" /></a>
<a href="https://huggingface.co/elatoumi"><img src="https://img.shields.io/badge/HuggingFace-ffb3d9?style=for-the-badge&logo=huggingface&logoColor=1a1023" /></a>
<a href="https://YOUR-PORTFOLIO-LINK.vercel.app"><img src="https://img.shields.io/badge/Portfolio-e88bc4?style=for-the-badge&logo=vercel&logoColor=1a1023" /></a>
<img src="https://komarev.com/ghpvc/?username=elatoumi&style=for-the-badge&color=ff8fc4&label=PROFILE+VIEWS" />

</div>

<br/>

## 🌸 About Me

```yaml
name: Ela Toumi
role: AI Engineering Student · Builder
location: Tunis, Tunisia 🇹🇳
school: ESPRIT Engineering School
degree: 3rd Year — AI Engineering (GPA 17.4/20)
collab: IRA Médenine (Institut des Régions Arides)
focus: [LLMs, Multi-Agent Systems, XAI, Applied Machine Learning]
currently: Building an AI research platform with ESPRIT × IRA Médenine
seeking: AI Engineering internships 🌸
```

<br/>

## 🌸 Featured Project — AI Research Platform

> A multi-module AI platform combining deep learning, large language models, and explainable AI — built in collaboration with **ESPRIT × IRA Médenine**, validated on real-world Tunisian data.

<table>
<tr>
<td width="50%" valign="top">

### 🧠 Module 8 — Predictive Deep Learning Model
- Transformer-based model (1B params) fine-tuned with lightweight adapters (IA³)
- AUC-ROC **0.9636** · AUPRC **0.9590** · F1 **0.8827**
- Explainability suite: attention heatmaps, Grad-CAM, embedding projections
- Deployed via **FastAPI** on 🤗 HuggingFace Spaces

</td>
<td width="50%" valign="top">

### 🔎 Module 7.1 — Semantic Search Engine
- Hybrid retrieval: dense embeddings + sparse features (RRF fusion)
- **MRR 0.78** · NDCG@5 0.81 · +90% precision vs keyword search
- RAG (LLaMA 3.1/Groq) + LangGraph multi-agent orchestration
- 4 query modes: raw input, ID, name, natural language

</td>
</tr>
</table>

<br/>

## ⚡ Tech Stack

<div align="center">

**Languages**
<br/>
![Python](https://img.shields.io/badge/Python-ffb3d9?style=for-the-badge&logo=python&logoColor=1a1023)
![Java](https://img.shields.io/badge/Java-ff9ecf?style=for-the-badge&logo=openjdk&logoColor=1a1023)
![JavaScript](https://img.shields.io/badge/JavaScript-f9a8d4?style=for-the-badge&logo=javascript&logoColor=1a1023)

<br/>

**AI / ML**
<br/>
![PyTorch](https://img.shields.io/badge/PyTorch-ff8fc4?style=for-the-badge&logo=pytorch&logoColor=1a1023)
![HuggingFace](https://img.shields.io/badge/🤗_HF_Transformers-f472b6?style=for-the-badge)
![LangChain](https://img.shields.io/badge/LangChain-ffb3d9?style=for-the-badge)
![LangGraph](https://img.shields.io/badge/LangGraph-ff9ecf?style=for-the-badge)
![XGBoost](https://img.shields.io/badge/XGBoost-e88bc4?style=for-the-badge)
![Scikit_learn](https://img.shields.io/badge/scikit--learn-f7a8d0?style=for-the-badge&logo=scikitlearn&logoColor=1a1023)

<br/>

**Backend & Deployment**
<br/>
![FastAPI](https://img.shields.io/badge/FastAPI-e88bc4?style=for-the-badge&logo=fastapi&logoColor=1a1023)
![Django](https://img.shields.io/badge/Django-ff8fc4?style=for-the-badge&logo=django&logoColor=1a1023)
![Docker](https://img.shields.io/badge/Docker-ffb3d9?style=for-the-badge&logo=docker&logoColor=1a1023)
![HuggingFace Spaces](https://img.shields.io/badge/🤗_Spaces-f472b6?style=for-the-badge)

<br/>

**Frontend & Tools**
<br/>
![React](https://img.shields.io/badge/React-f9a8d4?style=for-the-badge&logo=react&logoColor=1a1023)
![Next.js](https://img.shields.io/badge/Next.js-ff9ecf?style=for-the-badge&logo=nextdotjs&logoColor=1a1023)
![Git](https://img.shields.io/badge/Git-ffb3d9?style=for-the-badge&logo=git&logoColor=1a1023)
![Kaggle](https://img.shields.io/badge/Kaggle-ff8fc4?style=for-the-badge&logo=kaggle&logoColor=1a1023)
![LaTeX](https://img.shields.io/badge/LaTeX-e88bc4?style=for-the-badge&logo=latex&logoColor=1a1023)

</div>

<br/>

## 🎓 Certifications

<div align="center">

![NVIDIA](https://img.shields.io/badge/NVIDIA_Certified-×5-76b900?style=for-the-badge&logo=nvidia&logoColor=white)
![Azure](https://img.shields.io/badge/Azure_AI--103-in_progress-0078D4?style=for-the-badge&logo=microsoftazure&logoColor=white)

</div>

- 🟢 **5× NVIDIA Certifications** — Deep Learning & Applied AI
- 🔷 **Microsoft Azure AI-103** — *in progress*

<br/>

## 📊 GitHub Stats

<div align="center">
<img src="https://github-readme-stats.vercel.app/api?username=elatoumi&show_icons=true&theme=dracula&title_color=ff8fc4&icon_color=f472b6&text_color=f4c6dc&bg_color=1a1023&border_color=ff8fc4" width="48%"/>
<img src="https://github-readme-streak-stats.herokuapp.com/?user=elatoumi&theme=dark&background=1a1023&stroke=ff8fc4&ring=f472b6&fire=ff8fc4&currStreakLabel=ff8fc4&sideLabels=f4c6dc&dates=f4c6dc&border=ff8fc4" width="48%"/>
</div>

<br/>

<div align="center">
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:1a1023,50:3d1a2e,100:1a1023&height=100&section=footer"/>
</div>
